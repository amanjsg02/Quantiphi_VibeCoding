...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 bg-[#141414]/95 backdrop-blur-md border-b border-zinc-800/90 px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between gap-3 shadow-md">
      {/* Left: Sidebar Slide Toggle & Search Bar */}
      <div className="flex items-center gap-3 flex-1 max-w-lg">
        {/* Toggle Sidebar Button */}
        <button
          id="toggle-sidebar-btn"
          onClick={onToggleSidebar}
          className="p-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-700/80 transition-all flex items-center gap-1.5 shrink-0 shadow-sm cursor-pointer"
        ...
        <div className="relative w-full">
          ...
          <input
            id="global-food-search"
            type="text"
            placeholder="Search logged foods, categories, calories..."
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            className="w-full pl-9 pr-4 py-1.5 bg-zinc-900/90 border border-zinc-750 hover:border-zinc-600 focus:border-red-600 rounded-xl text-xs text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-red-600/30 transition-all font-medium"
          />
        </div>
      </div>

      {/* Right Controls: Subscription Icon, Add Meal, Notification Bell & User Account Switcher */}
      <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
        ...
        <button
          id="header-add-meal-btn"
          onClick={onOpenAddMealPage}
          className="inline-flex items-center gap-1.5 px-3 sm:px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-extrabold text-xs rounded-xl shadow-lg shadow-red-600/30 hover:shadow-red-600/50 transition-all cursor-pointer"
          title="Add a meal for the active user"
        >
            ...
            >
              ...
              <div className="pt-2 border-t border-zinc-800 space-y-1">
                ...
                >
                  ...
                  <span>Edit Profile / Add User</span>
                </button>
                <button
                  onClick={() => {
                    setShowUserMenu(false);
